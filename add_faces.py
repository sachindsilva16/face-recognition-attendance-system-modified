...
if name == "":
    ...
    while True: # Creating an infinite loop
        ...
        if k==ord('q') or k == ord('Q') or len(faces_data) == 50:  # Suppose if the user input pressed is 'q' or 'Q' then the loop will break(Exits).
            break
    ...
    faces_data=faces_data.reshape(50, -1) # This is basically to reshape the array.
    ...
    if 'names.pkl' not in os.listdir('data/'): # This is basically to check if the file is present or not.
        ...
        names=names+[name]*50 # Say suppose, the name "preethika" is already available in the pickle file, and u wanna add "priyal" then we just have to concatenate with other names.
